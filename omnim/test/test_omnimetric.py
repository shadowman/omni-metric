...
class TestOmniMetricCommandLineApp(tests.AppTest):
    ...
    def test_has_default_help_message(self):
        master_output = (
            "usage: main [-h] [-m METRICS] [-f INPUT_FILE]\n\n"
            "optional arguments:\n"
    ...
    def test_has_input_file_argument(self):
        status, app = self.runapp(
            self.app_cls,
            "omni-metric -f ./data/sample.csv"
        )
...
class TestOmniMetricCommandLineAppMetricsOutput(tests.AppTest):
    ...
    def test_runs_lead_time_metric_from_csv_file(self):
        ...
        with redirect_stdout(s_stdout):
            status, app = self.runapp(
                self.app_cls,
                "omni-metric -m lt -f ./data/sample.csv"
            )
    ...
    def test_runs_deployment_frequency_metric_from_csv_file(self):
        ...
        with redirect_stdout(s_stdout):
            status, app = self.runapp(
                self.app_cls,
                "omni-metric -m df -f ./data/sample.csv"
            )
    ...
    def test_runs_change_failure_rate_metric_from_csv_file(self):
        ...
        with redirect_stdout(s_stdout):
            status, app = self.runapp(
                self.app_cls,
                "omni-metric -m cfr -f ./data/sample.csv"
            )
    ...
    def test_runs_mean_time_to_restore_metric_from_csv_file_and_inform_not_enough_data(self):  # noqa: E501
        ...
        with redirect_stdout(s_stdout):
            status, app = self.runapp(
                self.app_cls,
                "omni-metric -m mttr -f ./data/sample.csv"
            )
    ...
    def test_runs_mean_time_to_restore_metric_from_csv_file(self):  # noqa: E501
        ...
        with redirect_stdout(s_stdout):
            status, app = self.runapp(
                self.app_cls,
                "omni-metric -m mttr -f ./data/mttr_data_stream.csv"
            )
